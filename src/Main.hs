...
showEither :: (Show a) => Either a a  -> String
showEither (Left l) = "Left of " ++ show l
showEither (Right r) = "Right of " ++ show r

data Point a = Point { x :: a, b :: a } deriving Show


sum :: (Monoid a) => [a] -> a
sum xs = foldl mappend mzero xs

main = do
    putStrLn "Hi"
    putStrLn $ show $ treeSingle 1